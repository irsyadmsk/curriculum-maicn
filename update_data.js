...
const jadwal = {
    senin_kamis: parseJadwal(jadwal_senin_kamis_raw),
    jumat: parseJadwal(jadwal_jumat_raw),
    sabtu: parseJadwal(jadwal_sabtu_raw)
};

data.program_tahunan = prota;
data.jadwal_harian = jadwal;

fs.writeFileSync('master-data.json', JSON.stringify(data, null, 1));
fs.writeFileSync('data.js', 'window.MASTER_DATA = ' + JSON.stringify(data, null, 1) + ';');
